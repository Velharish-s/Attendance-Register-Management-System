	...
	public void C(int a, Welcome obj) {
		...
		case 1:
			...
			while (b) {
				...
				infonormal.add("");
				infonormal_ar.add(-1);
				// System.out.print(status_P.size()); System.out.print(status_A.size());
				boolean k = true;
				while (k) {
					k = false;
		...
		case 6:// view attendance is common so it is end with login page
			if (name.size() != 0) {
				...
				if (p != -1) {
					...
					int sd = s2.nextInt();
					if (sd == 1) {
						System.out.println("Name: " + name.get(p) + " RegNo: " + regno.get(p) + " Total: "
								+ (status_P.get(p) + status_A.get(p)) + " Present: " + status_P.get(p) + " Absent: "
								+ status_A.get(p));
						System.out.println();
						d(obj);
					} 
						...
						System.out.println("Total: " + (status_P.get(p) + status_A.get(p)) + " Present: "
								+ status_P.get(p) + " Absent: " + status_A.get(p));
						System.out.println();
