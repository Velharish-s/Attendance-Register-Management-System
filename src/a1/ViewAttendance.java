...
public class ViewAttendance extends Welcome {
	...
	public static void attenview(int f, Welcome obj) {
		...
		int sd = o9.nextInt();
		if (sd == 1) {
			System.out.println("Name: " + name.get(f) + " RegNo: " + regno.get(f) + " Total: "
					+ (status_P.get(f) + status_A.get(f)) + " Present: " + status_P.get(f) + " Absent: "
					+ status_A.get(f));
			System.out.println();
			// obj.display(obj);
			...
			System.out.println("Total: " + (status_P.get(f) + status_A.get(f)) + " Present: " + status_P.get(f)
					+ " Absent: " + status_A.get(f));
			System.out.println();
